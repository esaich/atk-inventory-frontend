...
const ADMIN_MENU = [
    { 
    name: 'Dashboard', 
    path: '/admin', // Rute Dashboard Admin (Root)
    icon: (
...
export default function Sidebar({ user, onLogout, isOpen }) {
    ...
    // --- DEBUG LOG: PERIKSA NILAI PATH SAAT INI ---
    // Cek di console browser Anda. Jika di Dashboard Admin, nilainya HARUS '/'.
    // Jika nilainya bukan '/', ubah path di ADMIN_MENU[0] atau logic di bawah ini.
    console.log('Current Path from Router:', currentPath); 
    // ------------------------------------------------
    ...
    const isMenuActive = (path) => {
       // 1. Penanganan Dashboard Root ('/' atau '/divisi'): Harus SAMA PERSIS.
       // Ini mencegah Dashboard Admin menyala ketika di /admin/barang.
       if (path === '/admin' || path === '/divisi') {
          return currentPath === path;
       }

       // 2. Penanganan Modul Lain: Aktif jika sama persis ATAU merupakan sub-path (misal: /admin/barang/create)
